...
if [[ ${PV} == 9999 ]]; then
        inherit git-r3
        EGIT_REPO_URI="https://github.com/yquake2/rogue.git"
fi
...
#	local addon
#	for addon in $(usev ctf) $(usev rogue) $(usev xatrix); do
		insinto /usr/lib/yamagi-quake2/rogue
		doins "${WORKDIR}"/${PN}-${PV}/release/game.so
